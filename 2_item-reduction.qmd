...
```{r}
raw_data  <- read_csv("data/TPSai+-+Pilot+Tester.csv", n_max = 2)
colnames  <- names(raw_data)
q_text    <- setNames(sub(".* - ", "", as.character(raw_data[1, ])), colnames)
raw_data  <- read_csv("data/TPSai+-+Pilot+Tester.csv", col_names = colnames, skip = 3)
```
...
ggplot(plot_data_div, aes(x = pct_signed, y = response, fill = measure)) +
  geom_col(width = 0.7) +
